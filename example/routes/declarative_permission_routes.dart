...
/// 声明式权限配置使用示例
class DeclarativePermissionExamples {
  /// 示例1：使用预设权限配置
  static RouteConfig example1() {
    return RoutePresets.withDeclarativePermissions(
      '/example1',
      () => const DeclarativeCameraPage(),
      PermissionPresets.camera, // 使用预设配置
      title: '示例1：预设配置',
    );
  }

  /// 示例2：使用构建器模式
  static RouteConfig example2() {
    return RouteBuilder()
        .path('/example2')
        ...
        .withDeclarativePermissions(
          PermissionConfigBuilder()
              .required([AppPermission.location])
              .optional([AppPermission.storage])
              .description('使用构建器模式配置权限')
              .showGuide(true)
              .build(),
        )
        .build();
    ...
    return RouteBuilder()
        ...
        .withDeclarativePermissions(
          const RequiresPermissions(
            ...
            showGuide: true,
            allowSkipOptional: true,
            description: '复杂的自定义权限配置',
            customTitles: {
              AppPermission.camera: '专业相机',
              AppPermission.microphone: '高品质录音',
            },
            customDescriptions: {
              AppPermission.camera: '提供专业级拍摄功能',
              AppPermission.microphone: '支持高品质音频录制',
            },
          ),
        )
        .build();
